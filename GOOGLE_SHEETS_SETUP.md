...
This is a separate Google Sheet and service account from the groomsman site — don't reuse those credentials here.

## Step 1: Create a Google Spreadsheet

1. Go to [Google Sheets](https://sheets.google.com)
2. Click "Blank" to create a new spreadsheet
3. Name it "Bridesmaid Responses" (or whatever you prefer)
...
2. Create a new project (or select an existing one — a separate project from the groomsman site is cleanest, but not required)
...
4. Create a service account:
   ...
   - Name it "bridesmaid-responses" (or whatever you prefer)
...
2. Find the `client_email` field (looks like `bridesmaid-responses@project-name.iam.gserviceaccount.com`)
...
5. If deploying to a host that runs Node (Netlify, Render, Railway, etc.), add both variables in that host's environment variable settings too — the `.env.local` file only works locally.
